  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "Federated": "arn:aws:iam::${aws_vpc.eks_vpc.owner_id}:oidc-provider/${aws_iam_openid_connect_provider.oidc_provider.url}"
      },
      "Action": "sts:AssumeRoleWithWebIdentity",
      "Condition": {
        "StringEquals": {
          "${aws_iam_openid_connect_provider.oidc_provider.url}:sub": "system:serviceaccount:kube-system:alb-ingress-controller"
        }
      }
    }
  ]
